...
public:
  struct Element ///< This structure represents an ini-file entry
  {
    ...
    Element(TYPE type = TYPE::UNDEF, std::string section = "",
...
private:
  ...
  struct MemElement
      : Element ///< class Representing an Element within the memory
  {
    Line line; ///< the Line
  ...
  bool changed; ///< true if something has changed and we have to write it to
                ///< the file
public:
  ...
  /// same as flush
  /** but return false if no file is opened, or the file was opened in readonly
   * mode */
  bool write();
  ...
  */
  ...
  /// erases one element
  /** returns false if no file is open, the element is not correct or the
...
private:
  ...
  find_comment(const std::string &str); ///< finds a comment within a string

  /// finds the tag
  bool find_tag(std::string::size_type &start, std::string::size_type &end,
